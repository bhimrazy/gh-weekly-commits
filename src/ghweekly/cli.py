...
def main():
    parser = argparse.ArgumentParser(description="Fetch weekly GitHub commits for a user across multiple repos.")
    parser.add_argument('--username', required=True, help='GitHub username')
    parser.add_argument('--repos', nargs='+', required=True, help='List of GitHub repositories (org/repo)')
    parser.add_argument('--start', required=True, help='Start date (YYYY-MM-DD)')
    parser.add_argument('--end', default=datetime.now().strftime('%Y-%m-%d'), help='End date (YYYY-MM-DD)')
    parser.add_argument('--token', help='GitHub token (optional, for higher rate limits)')
    ...
    if args.plot:
        ax = df.plot(kind='bar', stacked=True, figsize=(14,6), colormap='tab20', width=0.8)
